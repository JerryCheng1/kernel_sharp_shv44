...
extern int sh_sched_load_boost_gold_plus;
extern unsigned int sh_sched_load_boost_freq_gold;
extern unsigned int sh_sched_load_boost_freq_gold_plus;
static int sh_get_sched_load_boost(int cpu)
{
	int ret = per_cpu(sched_load_boost, cpu);

	if (cpu >= 4 && cpu <= 6 && cpufreq_quick_get(cpu) >= sh_sched_load_boost_freq_gold)
		ret = sh_sched_load_boost_gold;
	if (cpu == 7 && cpufreq_quick_get(cpu) >= sh_sched_load_boost_freq_gold_plus)
		ret = sh_sched_load_boost_gold_plus;
