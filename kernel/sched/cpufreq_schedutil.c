...
static unsigned int sh_limit_freq_gold = 1804800;
module_param_named(
	sh_limit_freq_gold, sh_limit_freq_gold, int, S_IRUGO | S_IWUSR | S_IWGRP
);

static unsigned int sh_limit_freq_gold_plus = UINT_MAX;
module_param_named(
	sh_limit_freq_gold_plus, sh_limit_freq_gold_plus, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
static unsigned int sh_release_limit_enter_load = 43;
module_param_named(
...
static unsigned int sh_release_limit_exit_load = 43;
module_param_named(
...
static unsigned int sh_limit_fps_freq_gold = 1804800;
module_param_named(
	sh_limit_fps_freq_gold, sh_limit_fps_freq_gold, int, S_IRUGO | S_IWUSR | S_IWGRP
);

static unsigned int sh_limit_fps_freq_gold_plus = UINT_MAX;
module_param_named(
	sh_limit_fps_freq_gold_plus, sh_limit_fps_freq_gold_plus, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
static void sh_release_limit(struct sugov_cpu *sg_cpu, unsigned long util)
{
	unsigned int release_limit_freq[NR_CPUS];
	int cpu, scale_cpu;
	struct cpufreq_policy *policy_scale;
	u64 sh_cpu_util_calc = 0, cpu_util = 0;
	u64 release_limit_enter_base = 0, release_limit_exit_base = 0;
	u64 elapsed_time_enter, elapsed_time_exit;
	u64 now;

	for_each_possible_cpu(cpu) {
		release_limit_freq[cpu] = max(per_cpu(sh_load_limit_freq, cpu), per_cpu(sh_fps_limit_freq, cpu));
	...
	scale_cpu = 7;
	policy_scale = sh_cpufreq_cpu_get_raw(scale_cpu);
	if (policy_scale) {
		sh_cpu_util_calc = mult_frac(util, policy_scale->cpuinfo.max_freq, sg_cpu->sg_policy->policy->cpuinfo.max_freq) * 100;
		sh_cpu_util_calc = sh_cpu_util_calc / topology_get_cpu_scale(NULL, scale_cpu);
	}
	per_cpu(sh_cpu_util, sg_cpu->cpu) = sh_cpu_util_calc;

	for_each_online_cpu(cpu) {
		if (sh_load_sum_mode == 0)
			cpu_util = cpu_util + per_cpu(sh_cpu_util, cpu);
		if (sh_load_sum_mode == 1) {
			if (cpu >= 4)
				cpu_util = cpu_util + per_cpu(sh_cpu_util, cpu);
		}
		if (sh_load_sum_mode == 2)
			cpu_util = max((unsigned int)cpu_util, per_cpu(sh_cpu_util, cpu));
	}

	if (cpu_util < sh_release_limit_enter_load || release_limit_enter_base == 0)
		per_cpu(sh_release_limit_enter_base, sg_cpu->cpu) = release_limit_enter_base = now;
	...
	if (cpu_util >= sh_release_limit_exit_load || release_limit_exit_base == 0)
		per_cpu(sh_release_limit_exit_base, sg_cpu->cpu) = release_limit_exit_base = now;
	...
	if (elapsed_time_exit >= sh_release_limit_exit_time) {
		for_each_possible_cpu(cpu) {
			...
			if (cpu == 7)
				per_cpu(sh_load_limit_freq, cpu) = sh_limit_freq_gold_plus;
		}
	}
	...
	if (sh_disp_on && drm_base_fps_low_mode() > 60) {
		for_each_possible_cpu(cpu) {
			...
			if (cpu == 7)
				per_cpu(sh_fps_limit_freq, cpu) = sh_limit_fps_freq_gold_plus;
		}
	} else {
		for_each_possible_cpu(cpu)
	...
	if (!sh_update_policy_try) {
		for_each_possible_cpu(cpu) {
			if (release_limit_freq[cpu] != max(per_cpu(sh_load_limit_freq, cpu), per_cpu(sh_fps_limit_freq, cpu))) {
				sh_update_policy_try = true;
				break;
			}
		}
	...
	if (data) {
		if (evdata->id == MSM_DRM_PRIMARY_DISPLAY) {
			if (event == MSM_DRM_EVENT_DPMS) {
				...
				switch (blank) {
				case MSM_DRM_EVENT_DPMS_ON:
					sh_disp_on = true;
					break;
				case MSM_DRM_EVENT_DPMS_OFF:
					sh_disp_on = false;
					break;
				default:
					break;
