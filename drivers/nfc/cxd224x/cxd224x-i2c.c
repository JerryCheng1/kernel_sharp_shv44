...
#endif  // CONFIG_NFC_CXD224X_TST1
    unsigned int status_gpio;   // Add NFC_STATUS pin
    bool irq_enabled;
    struct mutex lock;
...
#endif
    struct pinctrl_state    *pin_default_status;    // Add NFC_STATUS pin
};
                     ...
                     unsigned int cmd, unsigned long arg)
{
    ...
    case CXDNFC_TST1_CTL:
        ...
        break;
#endif  // CONFIG_NFC_CXD224X_TST1
    case CXDNFC_GET_NINT:
        NFC_DRV_DBG_LOG("get nint");
        ret = gpio_get_value(cxd224x_dev->irq_gpio);
        if(copy_to_user((unsigned int *)arg, &ret, sizeof(unsigned int)) != 0) {
            NFC_DRV_ERR_LOG("copy failed to user");
        }
        break;
    case CXDNFC_GET_STATUS:
        NFC_DRV_DBG_LOG("get status");
        ret = gpio_get_value(cxd224x_dev->status_gpio);
        if(copy_to_user((unsigned int *)arg, &ret, sizeof(unsigned int)) != 0) {
            NFC_DRV_ERR_LOG("copy failed to user");
        }
        break;
...
static struct cxd224x_platform_data *cxd224x_get_dts_config(struct device *dev)
{
    ...
    // Add NFC_STATUS pin Start
    pdata->status_gpio = of_get_named_gpio(node, "cxd224x,status_gpio", 0);
    if (!gpio_is_valid(pdata->status_gpio)) {
        ret = -EINVAL;
        DBGLOG_ERR("failed to of_get_named_gpio(cxd224x,status_gpio(%d))\n",pdata->status_gpio);
        goto err;
    }
    // Add NFC_STATUS pin End
...
#endif
    // Add NFC_STATUS pin Start
    pdata->pin_default_status = pinctrl_lookup_state(pdata->pinctrl, "cxd224x_default_status");
    if (IS_ERR_OR_NULL(pdata->pin_default_status)) {
        ret = -EINVAL;
        DBGLOG_ERR("Failed to look up default state\n");
        goto err;
    }
    ...
    DBGLOG_DETAIL("[%s(),line(%d)] pdata->status_gpio(%d)\n",__func__, __LINE__, pdata->status_gpio);   // Add NFC_STATUS pin
               ...
               const struct i2c_device_id *id)
{
    ...
    // Add NFC_STATUS pin Start
    ret = pinctrl_select_state(platform_data->pinctrl, platform_data->pin_default_status);
    if (ret) {
        DBGLOG_ERR("probing cxd224x driver flagsCan't select pinctrl state(%d)\n", ret);
        goto err_exit;
    }
    ret = gpio_request_one(platform_data->status_gpio, GPIOF_IN, "nfc_status");
    if (ret){
        DBGLOG_ERR("failed gpio_reques(nfc_status) ret(%d)\n", ret);
        goto err_exit;
    }
    status_gpio_ok=1;
    // Add NFC_STATUS pin End
...
#endif  // CONFIG_NFC_CXD224X_TST1
    cxd224x_dev->status_gpio = platform_data->status_gpio;  // Add NFC_STATUS pin
...
#endif
    cxd224x_dev->pin_default_status = platform_data->pin_default_status;    // Add NFC_STATUS pin
    ...
    DBGLOG_DETAIL("[%s(),line(%d)]  status value(%d)\n",__func__, __LINE__, gpio_get_value(platform_data->status_gpio) );   // Add NFC_STATUS pin
...
err_exit:
    ...
    // Add NFC_STATUS pin Start
    if(status_gpio_ok)
        gpio_free(platform_data->status_gpio);
    // Add NFC_STATUS pin End
...
static int cxd224x_remove(struct i2c_client *client)
{
    ...
    gpio_free(cxd224x_dev->status_gpio);    // Add NFC_STATUS pin
