 ...
 */
#ifdef CONFIG_BATTERY_SHARP
struct sysfs_link_device {
	char name[THERMAL_NAME_LENGTH]; //thermal_sensors label in device-tree node
	char type[THERMAL_NAME_LENGTH]; //linked sysfs type
};
static struct sysfs_link_device link_device_table[] = {
	/* name,	type*/
	{"battery",				"battery"},
	{"camera-flash-therm",	"cam_therm"},
	{"lcd-therm",			"lcd_therm"},
	{"skin-msm-therm",		"msm_therm"},
	{"pa-therm1",			"pa_therm1"},
	{"pm8150_tz",			"pmic_therm"},
	{"skin-therm",			"skin_therm"},
	{"usb-therm",			"usb_therm"},
	{"xo-therm",			"xo_therm"},
	{"phaser1-right-therm",	"phaser1_right_therm"},
	{"modem1-skin-usr",		"modem1_skin_usr"},
	{"modem1-pa0-usr",		"modem1_pa0_usr"},
	{"modem1-pa1-usr",		"modem1_pa1_usr"},
	{"modem1-pa2-usr",		"modem1_pa2_usr"},
	{"wp-therm",			"aux_therm"}
};
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	for (index = 0; index < sizeof(link_device_table)/sizeof(struct sysfs_link_device); index++) {
		if (!strcmp(tz->type, link_device_table[index].name)) {
			if (sysfs_create_link(&(tz->device.class)->p->subsys.kobj, &tz->device.kobj, link_device_table[index].type)) {
				pr_debug("fail to create link sys file:%s\n", link_device_table[index].type);
			}
			break;
		}
	}
