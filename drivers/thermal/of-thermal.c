...
int __init of_parse_thermal_zones(void)
{
	...
	for_each_available_child_of_node(np, child) {
		...
		if (!of_property_read_string(child, "thermal-governor",
						&governor_name)){
			if(!strncmp(governor_name, "user_space", sizeof("user_space") - 1)
			 && strncmp(child->name, "cpu-0-0-usr", sizeof("cpu-0-0-usr") - 1)
			 && strncmp(child->name, "cpu-1-0-usr", sizeof("cpu-1-0-usr") - 1)
			 && strncmp(child->name, "cpu-1-3-usr", sizeof("cpu-1-3-usr") - 1)){
				strlcpy(tzp->governor_name, "step_wise", THERMAL_NAME_LENGTH);
			}else{
