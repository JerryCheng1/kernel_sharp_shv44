...
int shpwr_dbg_log_add_file(void)
{
    ...
    if (shpwr_dbg_kernel_seek(fp, 0, SEEK_END)
        ...
        do {
            ...
            while(1) {
                res = vfs_read(fp, copy_bufp, (size_t)SHPWR_DBG_LOG_COPY_BUF_SIZE, &fp->f_pos);
                if (!res) {
                    /* read file end */
                    break;
                }
                res = vfs_write(fp_old, copy_bufp, (size_t)res, &fp_old->f_pos);
                if (!res) {
...
int shpwr_dump_reg_add_file(void)
{
    ...
    if (shpwr_dbg_kernel_seek(fp, 0, SEEK_END)
        ...
        do {
            ...
            while(1) {
                res = vfs_read(fp, copy_bufp, (size_t)SHPWR_DUMP_REG_COPY_BUF_SIZE, &fp->f_pos);
                if (!res) {
                    /* read file end */
                    break;
                }
