...
#ifdef CONFIG_BATTERY_SHARP
	rc = smblib_get_iio_channel(chg, "usb_therm", &chg->iio.usb_therm_chan);
	if (rc < 0)
		return rc;
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_BATTERY_CHARGER_STATUS,
	POWER_SUPPLY_PROP_VOLTAGE_MAX_AGED,
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_ERROR_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGER_STATUS:
		rc = smblib_get_prop_batt_charger_status(chg, val);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_AGED:
		val->intval = get_client_vote_locked(chg->fv_votable, FV_AGED_VOTER);
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CC_SAFETY_LEVEL:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_AGED:
		vote(chg->fv_votable, FV_AGED_VOTER, val->intval > 0, val->intval);
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
		rc = -EINVAL;
...
#ifdef CONFIG_BATTERY_SHARP
	/* set DC icl_max 0mA */
	rc = smblib_set_charge_param(chg, &chg->param.dc_icl, 0);
#else
	/* set DC icl_max 1A */
...
#ifdef CONFIG_BATTERY_SHARP
static int smb5_shdiag_mode_initialize(const char *val, const struct kernel_param *kp)
{
	int rc = 0;
...
static int battery_charging_enabled_set(const char *val, const struct kernel_param *kp)
{
    int ret,value;
    struct smb_charger *chg = &the_chip->chg;
...
static int battery_charging_enabled_get(char *buf, const struct kernel_param *kp)
{
    int ret, value;
    struct smb_charger *chg = &the_chip->chg;
...
static int input_current_max_set(const char *val, const struct kernel_param *kp)
{
    int ret,value;
    struct smb_charger *chg = &the_chip->chg;
...
static int input_current_max_get(char *buf, const struct kernel_param *kp)
{
    int ret, value;
    struct smb_charger *chg = &the_chip->chg;
