...
#ifdef CONFIG_BATTERY_SHARP
static int current_avg_update_period_ms = 30000;
module_param_named(
	current_avg_period_ms, current_avg_update_period_ms, int, S_IRUSR | S_IWUSR
);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
static int fg_gen4_power_supply_changed(void *data)
{
	struct fg_gen4_chip *chip = data;
	struct fg_dev *fg = &chip->fg;
...
static int fg_gen4_get_batt_profile(struct fg_dev *fg)
{
...
#ifdef CONFIG_BATTERY_SHARP
	{
		...
		profile_node = of_batterydata_get_best_profile(batt_node,
					fg->batt_id_ohms / 1000, batt_type);
	}
#else
		profile_node = of_batterydata_get_best_profile(batt_node,
	...
	rc = of_property_read_u32(profile_node, "qcom,nom-batt-capacity-mah",
			&fg->bp.nom_batt_cap_mah);
	if (rc < 0) {
		pr_err("battery nominal capacity unavailable, rc:%d\n", rc);
		fg->bp.nom_batt_cap_mah = -EINVAL;
	}
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	fg_dbg(fg, FG_STATUS, "recharge: %d, input: %d\n", recharge_soc, is_input_present(fg));
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
		if (msoc >= 97 && (fg->health == POWER_SUPPLY_HEALTH_GOOD || fg->health == POWER_SUPPLY_HEALTH_COOL)) {
...
void cpu_clock_limit_lock(struct fg_dev *fg)
{
	int cpu;
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);

	fg_dbg(fg, FG_STATUS, "Entered\n");
...
void cpu_clock_limit_unlock(struct fg_dev *fg)
{
	int cpu;
...
static irqreturn_t fg_empty_soc_irq_handler(int irq, void *data)
{
	struct fg_dev *fg = data;
#if 0
/* COORDINATOR Qualcomm_CS2.4/PreCS2.1 BUILDERR MODIFY start */
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
/* COORDINATOR Qualcomm_CS2.4/PreCS2.1 BUILDERR MODIFY end */
#endif
	fg_dbg(fg, FG_IRQ, "irq %d triggered\n", irq);
...
//	if (is_batt_empty(fg)) {
	if (chip->vbatt_low) {
/* COORDINATOR Qualcomm_CS2.4/PreCS2.1 BUILDERR MODIFY end */
		cpu_clock_limit_lock(fg);
	} else {
		cpu_clock_limit_unlock(fg);
	}
#endif /* CONFIG_BATTERY_SHARP */
...
static void update_current_avg(struct fg_gen4_chip *chip)
{
	int i, rc = 0;
	...
	if (!rc) {
		if (!call_once) {
			...
			fg_get_battery_current(&chip->fg, &temp_current);
...
static void update_current_avg_work(struct work_struct *work)
{
	...
	schedule_delayed_work(
		...
		msecs_to_jiffies(current_avg_update_period_ms));
}
#endif /* CONFIG_BATTERY_SHARP */
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_IBATT_FULL:
		...
		break;
	case POWER_SUPPLY_PROP_VBATT_FULL:
		fg->bp.vbatt_full_mv = (pval->intval);
		if (fg->bp.vbatt_full_mv > 0) {
			rc = fg_set_constant_chg_voltage(fg,
					fg->bp.vbatt_full_mv * 1000);
			if (rc < 0)
				pr_err("Error in writing constant_chg_voltage, rc=%d\n", rc);
		}
		break;
#endif /* CONFIG_BATTERY_SHARP */
	default:
...
static enum power_supply_property fg_psy_props[] = {
	...
	POWER_SUPPLY_PROP_IBATT_FULL,
	POWER_SUPPLY_PROP_VBATT_FULL,
#endif /* CONFIG_BATTERY_SHARP */
};
...
#ifdef CONFIG_BATTERY_SHARP
	cl->power_supply_changed = fg_gen4_power_supply_changed;
#endif /* CONFIG_BATTERY_SHARP */
	cl->data = chip;
...
static void fg_gen4_cleanup(struct fg_gen4_chip *chip)
{
	...
	cancel_work_sync(&chip->pl_current_en_work);
#ifdef CONFIG_BATTERY_SHARP
	cancel_delayed_work_sync(&chip->update_current_avg_work);
#endif /* CONFIG_BATTERY_SHARP */
...
static int fg_gen4_probe(struct platform_device *pdev)
{
	...
	{
		fg_dbg(fg, FG_STATUS, "cpufreq_register_notifier succeeded.\n");
	}
#endif /* CONFIG_BATTERY_SHARP */
...
static int fg_gen4_resume(struct device *dev)
{
	...
	next_update_time = chip->last_current_avg_update_time
		+ (current_avg_update_period_ms / 1000);
