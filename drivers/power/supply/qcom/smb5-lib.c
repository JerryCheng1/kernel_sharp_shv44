...
#ifdef CONFIG_BATTERY_SHARP
int smblib_set_dc_pulldown(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	u8 mask = (DCIN_PULLDOWN_DC_IN_BIT | DCIN_PULLDOWN_DC_MID_BIT);

	smblib_dbg(chg, PR_REGISTER, "dcin cmd pulldown : %d\n", enable);

	rc = smblib_masked_write(chg, DCIN_CMD_PULLDOWN_REG, mask,
				enable ? mask : 0);
	if (rc < 0)
		smblib_err(chg, "Couldn't write 0x%x to DCIN_PULLDOWN DC_IN and DC_MID rc=%d\n",
			enable ? mask : 0, rc);

	return rc;
}

#define REWRITE_NUM_DCIN_CMD_PULLDOWN_REG			3
#define REWRITE_INTERVAL_MS_DCIN_CMD_PULLDOWN_REG	50	/* ms */
static void dcin_reverse_boost_wa(struct smb_charger *chg)
{
	int rc;
	int count;
	u8 mask = (DCIN_EN_VALUE_BIT | DCIN_EN_OVERRIDE_BIT);

	smblib_dbg(chg, PR_INTERRUPT, "start dcin_reverse_boost_wa() execute:%d\n", chg->dcin_reverse_boost_wa_flag);

	if (chg->dcin_reverse_boost_wa_flag) {
		smblib_dbg(chg, PR_INTERRUPT, "dcin_reverse_boost_wa() executing\n");
		return;
	}

	// Workaround
	chg->dcin_reverse_boost_wa_flag = true;
	smblib_set_dc_suspend(chg, true);

	rc = smblib_masked_write(chg, DCIN_CMD_IL_REG, mask, DCIN_EN_OVERRIDE_BIT);
	if (rc < 0)
		smblib_err(chg, "Couldn't write DCIN EN_VALUE and EN_OVERRIDE val=0x%x rc=%d\n", DCIN_EN_OVERRIDE_BIT, rc);

	for (count = 0; count < REWRITE_NUM_DCIN_CMD_PULLDOWN_REG; count++) {
		smblib_set_dc_pulldown(chg, true);
		msleep(REWRITE_INTERVAL_MS_DCIN_CMD_PULLDOWN_REG);
	}

	smblib_set_dc_pulldown(chg, false);

	rc = smblib_masked_write(chg, DCIN_CMD_IL_REG, mask, 0);
	if (rc < 0)
		smblib_err(chg, "Couldn't write DCIN EN_VALUE and EN_OVERRIDE val=0x%x rc=%d\n", 0, rc);

	smblib_set_dc_suspend(chg, false);

	chg->dcin_reverse_boost_wa_flag = false;
	smblib_dbg(chg, PR_INTERRUPT, "end dcin_reverse_boost_wa() execute:%d\n", chg->dcin_reverse_boost_wa_flag);

	cancel_delayed_work_sync(&chg->weak_dc_charger_wa);
}
#endif /* CONFIG_BATTERY_SHARP */
...
int smblib_get_prop_batt_charger_status(struct smb_charger *chg,
				    union power_supply_propval *val)
{
	int rc = 0;
	u8 stat = 0;
	...
	switch (stat & BATTERY_CHARGER_STATUS_MASK) {
	case INHIBIT_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_INHIBIT;
		break;
	case TRICKLE_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_TRICKLE;
		break;
	case PRE_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_PRECHARGE;
		break;
	case FULLON_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_FULLON;
		break;
	case TAPER_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_TAPER;
		break;
	case TERMINATE_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_TERMINATION;
		break;
	case PAUSE_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_PAUSE_CHARGE;
		break;
	case DISABLE_CHARGE:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_DISABLE_CHARGE;
		break;
	default:
		val->intval = POWER_SUPPLY_BATTERY_CHG_STS_NONE;
	}
...
#define REATTACH_USB_WAIT_TIME 1000
int smblib_detach_usb_in_init(struct smb_charger *chg)
{
	...
	vote(chg->cc_disable_votable, DETACH_ATTACH_USB_VOTER, true, 0);
	schedule_delayed_work(&chg->reattach_usb_work,
					msecs_to_jiffies(REATTACH_USB_WAIT_TIME));
	return 0;
}
#endif /* CONFIG_BATTERY_SHARP */
				    ...
				    union power_supply_propval *val)
{
	...
	if (!rc) pval.intval |= pval_dc.intval;
#else
	rc = smblib_get_prop_usb_present(chg, &pval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't get usb presence status rc=%d\n", rc);
...
#ifdef CONFIG_BATTERY_SHARP
irqreturn_t dcin_revi_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_INTERRUPT, "DCIN_RT_STS = 0x%x\n", stat);

	if (stat & DCIN_REVI_RT_STS_BIT) {
		dcin_reverse_boost_wa(chg);
	}

	return IRQ_HANDLED;
}

#define WEAK_DC_CHARGER_WA_DELAY	1000    //1000[ms]
irqreturn_t dcin_en_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
	int rc = 0;
	...
	if (!(stat & DCIN_EN_RT_STS_BIT)) {
	schedule_delayed_work(&chg->weak_dc_charger_wa,
					msecs_to_jiffies(WEAK_DC_CHARGER_WA_DELAY));
	}
...
irqreturn_t dcin_uv_irq_handler(int irq, void *data)
{
	...
	if (is_storming(&irq_data->storm_data)){
		smblib_dbg(chg, PR_MISC, "DCIN_UV storm occurred\n");
		vote(chg->dc_suspend_votable, DC_AICL_THRESHOLD_VOTER, true, 0);
	}
...
irqreturn_t usbin_uv_irq_handler(int irq, void *data)
{
	...
	if(!(chg->wa_flags & WEAK_ADAPTER_WA)
		...
		smblib_dbg(chg, PR_MISC, "USBIN_UV storm occurred\n");
...
irqreturn_t dc_plugin_irq_handler(int irq, void *data)
{
	...
	int sec_charger;
#ifdef CONFIG_BATTERY_SHARP
	static bool prev_dcin_present = false;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	vote(chg->awake_votable, DC_SRC_VOTER, true, 0);
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	if (prev_dcin_present && !dcin_present) {
		vote(chg->dc_suspend_votable, DC_AICL_THRESHOLD_VOTER, false, 0);
		cancel_delayed_work_sync(&chg->weak_dc_charger_wa);
		dcin_reverse_boost_wa(chg);
		vote(chg->awake_votable, DC_SRC_VOTER, false, 0);
	}
	prev_dcin_present = dcin_present;
#endif /* CONFIG_BATTERY_SHARP */
...
int smblib_get_usb_therm(struct smb_charger *chg, int *usb_therm)
{
	...
	if(chg->iio.usb_therm_chan == 0)
	{
		rc = -EINVAL;
		smblib_err(chg, "[E] %s() ret:%d usb_therm_chan:0x%x is not set.\n",
				__FUNCTION__, rc, chg->iio.usb_therm_chan );
		return rc;
	}

	rc = smblib_read_iio_channel(chg, chg->iio.usb_therm_chan, 0, usb_therm);
	if(rc < 0)
	{
		smblib_err(chg, "[E] %s() ret:%d smblib_read_iio_channel() ch:0x%x is failed.\n",
					__FUNCTION__, rc, chg->iio.usb_therm_chan );
		return rc;
	}
...
#define BATTERY_CHARGING_PAUSE_BIT_MASK		0x07
#define BATTERY_CHARGING_PAUSE_BIT			0x06
#define DCIN_PON_AND_PLUGIN_MASK			0xD0
#define DCIN_PON_AND_PLUGIN					0x50
static void smblib_weak_dc_charger_wa(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						weak_dc_charger_wa.work);
	int rc = 0;
	u8 stat;
	u8 stat_vat_chg;
	u8 stat_dc_sts;
	...
	stat_vat_chg = stat & BATTERY_CHARGING_PAUSE_BIT_MASK;

	// Check dcin_en = 0 && dcin_pon=1 && dcin_plugin = 1
	rc = smblib_read(chg, DCIN_BASE + INT_RT_STS_OFFSET, &stat);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read DCIN_RT_STS rc=%d\n", rc);
		return;
	...
	stat_dc_sts = stat & DCIN_PON_AND_PLUGIN_MASK;

	if((stat_vat_chg == BATTERY_CHARGING_PAUSE_BIT) && (stat_dc_sts == DCIN_PON_AND_PLUGIN)){
		//Force DC_IN_EN high
		smblib_set_dc_suspend(chg, true);

		//Suspend DC_IN path
		mask = (DCIN_EN_VALUE_BIT | DCIN_EN_OVERRIDE_BIT);
		rc = smblib_masked_write(chg, DCIN_CMD_IL_REG, mask, DCIN_EN_OVERRIDE_BIT);
		if (rc < 0)
			smblib_err(chg, "Couldn't DCIN_CMD_IL_REG rc=%d\n", rc);

		smblib_dbg(chg, PR_MISC, "%s Force DCIN_EN high and suspend DCIN path.\n", __func__);
	}

	return;
}

static void smblib_reattach_usb_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
					reattach_usb_work.work);

	vote(chg->cc_disable_votable, DETACH_ATTACH_USB_VOTER, false, 0);
}
#endif /* CONFIG_BATTERY_SHARP */
...
int smblib_init(struct smb_charger *chg)
{
...
#ifdef CONFIG_BATTERY_SHARP
	INIT_DELAYED_WORK(&chg->reattach_usb_work, smblib_reattach_usb_work);
#endif /* CONFIG_BATTERY_SHARP */
	...
	INIT_DELAYED_WORK(&chg->weak_dc_charger_wa, smblib_weak_dc_charger_wa);
...
#ifdef CONFIG_BATTERY_SHARP
		cancel_delayed_work_sync(&chg->typec_usb_therm_work);
		cancel_delayed_work_sync(&chg->reattach_usb_work);
		cancel_delayed_work_sync(&chg->weak_dc_charger_wa);
#endif /* CONFIG_BATTERY_SHARP */
