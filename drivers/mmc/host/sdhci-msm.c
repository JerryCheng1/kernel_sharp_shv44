...
sdhci_msm_set_enpwr_gpio(struct sdhci_msm_host *msm_host, bool enable)
{
	...
	if (!gpio_is_valid(sdpwr_en)) {
		pr_debug("%s: sdpwr-gpio resource error\n",
	...
	if (enable) {
		if(mmc_gpio_get_cd(msm_host->mmc) == 1) {
			...
			if (rc) {
				pr_err("%s: enable request for sdpwr_en_gpio failed, rc=%d\n",
					mmc_hostname(msm_host->mmc), rc);
			} else {
		...
		rc = gpio_request(sdpwr_en, "sdpwr_en_gpio");
		if (rc) {
			pr_err("%s: disable request for sdpwr_en_gpio failed, rc=%d\n",
				mmc_hostname(msm_host->mmc), rc);
		} else {
			gpio_set_value_cansleep(sdpwr_en, enable);
			gpio_free(sdpwr_en);
