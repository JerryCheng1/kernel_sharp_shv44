...
#define SHTIMER_NONE_SIG   0x00000000UL
#define SHTIMER_CANCEL_SIG 0x00000001UL
#define SHTIMER_EXPIRE_SIG 0x00000002UL

#define SLEEP_NEGATE_TIME 50000000LL
...
static long shtimer_do_ioctl(struct file *file, unsigned int cmd, struct timespec *tv)
{
	...
	case SHTIMER_CMD_WAIT:
		spin_lock_irqsave(&shtimer_slock, flags);
		if((cmd == SHTIMER_CMD_SET) && (SLEEP_NEGATE_TIME < list->tv)){
