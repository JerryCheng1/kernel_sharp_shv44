	...
	int power_mode)
{
	...
	case SDE_MODE_DPMS_ON:
		blank = MSM_DRM_EVENT_DPMS_ON;
		break;
	case SDE_MODE_DPMS_LP1:
		blank = MSM_DRM_EVENT_DPMS_LP1;
		break;
	case SDE_MODE_DPMS_LP2:
		blank = MSM_DRM_EVENT_DPMS_LP2;
		break;
	case SDE_MODE_DPMS_OFF:
		blank = MSM_DRM_EVENT_DPMS_OFF;
		break;
	default:
		return;
	}
	...
	notifier_data.id =
		connector->state->crtc->index;
	msm_drm_notifier_call_chain(MSM_DRM_EVENT_DPMS,
					    &notifier_data);
}
#endif /* CONFIG_SHARP_DISPLAY */
