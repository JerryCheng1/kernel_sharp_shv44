...
#include "msm_kms.h"
#include "sde_wb.h"
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00004 */ /* CUST_ID_00005 */ /* CUST_ID_00039 */
#include <soc/qcom/sh_smem.h>
#include "sharp/msm_drm_context.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00006 */
#include "dsi_display.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00013 */ /* CUST_ID_00029 */ /* CUST_ID_00031 */
#include "sharp/drm_diag.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00032 */
#include "sharp/drm_cmn.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
...
int msm_drm_check_panel_connected(struct msm_drm_private *priv);
#endif /* CONFIG_SHARP_DISPLAY */
...
static int msm_ioctl_get_flicker_param(struct drm_device *dev, void *data,
		struct drm_file *file);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00029 */
...
static int msm_ioctl_mipi_dsi_mipichk(struct drm_device *dev, void *data,
		struct drm_file *file);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00039 */
...
static int msm_drv_fpslow_base = DRM_BASE_FPS_120;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00004 */
	msm_drm_check_panel_connected(priv);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00005 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00055 */
		if (event->type == DRM_EVENT_PANEL_OFF) {
...
int msm_drm_check_panel_connected(struct msm_drm_private *priv)
{
	...
	priv->panel_is_connected = shdisp_boot_ctx->panel_connected;
	if (!priv->panel_is_connected) {
		pr_info("%s:panel is not connected\n", __func__);
	}

	pr_debug("check_connected_panel connected=%d\n", priv->panel_is_connected);
	return 0;
}
#endif /* CONFIG_SHARP_DISPLAY */
...
int msm_drm_check_upper_unit(struct msm_drm_private *priv)
{
	...
	if (!val) {
		pr_info("%s:upper_unit is not connected\n", __func__);
		priv->upper_unit_is_connected = DRM_UPPER_UNIT_IS_NOT_CONNECTED;
	}
...
struct dsi_display *msm_drm_get_dsi_displey(void)
{
	...
	struct dsi_display *display_array[MAX_DSI_ACTIVE_DISPLAY];
	if (msm_drm_primary_display) {
		return msm_drm_primary_display;
	}

	count = dsi_display_get_active_displays((void**)display_array, MAX_DSI_ACTIVE_DISPLAY);
	if (count) {
		display = display_array[DSI_PRIMARY];
		pr_info("%s:%s\n",__func__,display->name);
	} else {
		pr_err("%s: dsi_display_get_active_displays is 0.\n", __func__);
		return NULL;
	}
...
static void msm_mdp_mipi_clkchg_param_log(struct mdp_mipi_clkchg_param *clkchg_param)
{
	...
	pr_debug("[%s]param->host.timing_ctrl[12]   = 0x%02X\n",
			__func__, clkchg_param->host.timing_ctrl[12]  );
	pr_debug("[%s]param->host.timing_ctrl[13]   = 0x%02X\n",
			__func__, clkchg_param->host.timing_ctrl[13]  );

	pr_debug("[%s]param->panel.vbp[0]           = 0x%02X\n",
			__func__, clkchg_param->panel.vbp[0]          );
	pr_debug("[%s]param->panel.vbp[1]           = 0x%02X\n",
			__func__, clkchg_param->panel.vbp[1]          );
	pr_debug("[%s]param->panel.vfp[0]           = 0x%02X\n",
			__func__, clkchg_param->panel.vfp[0]          );
	pr_debug("[%s]param->panel.vfp[1]           = 0x%02X\n",
			__func__, clkchg_param->panel.vfp[1]          );
	pr_debug("[%s]param->panel.slt[0]           = 0x%02X\n",
			__func__, clkchg_param->panel.slt[0]          );
	pr_debug("[%s]param->panel.slt[1]           = 0x%02X\n",
			__func__, clkchg_param->panel.slt[1]          );
	pr_debug("[%s]param->panel.vbp_2c[0]        = 0x%02X\n",
			__func__, clkchg_param->panel.vbp_2c[0]       );
	pr_debug("[%s]param->panel.vbp_2c[1]        = 0x%02X\n",
			__func__, clkchg_param->panel.vbp_2c[1]       );
	pr_debug("[%s]param->panel.vfp_2c[0]        = 0x%02X\n",
			__func__, clkchg_param->panel.vfp_2c[0]       );
	pr_debug("[%s]param->panel.vfp_2c[1]        = 0x%02X\n",
			__func__, clkchg_param->panel.vfp_2c[1]       );
	pr_debug("[%s]param->panel.slt_2c[0]        = 0x%02X\n",
			__func__, clkchg_param->panel.slt_2c[0]       );
	pr_debug("[%s]param->panel.slt_2c[1]        = 0x%02X\n",
			__func__, clkchg_param->panel.slt_2c[1]       );
	pr_debug("[%s]param->panel.stv_on_2c        = 0x%02X\n",
			__func__, clkchg_param->panel.stv_on_2c       );
	pr_debug("[%s]param->panel.stv_off_2c       = 0x%02X\n",
			__func__, clkchg_param->panel.stv_off_2c      );
	pr_debug("[%s]param->panel.ckv0_on_2c       = 0x%02X\n",
			__func__, clkchg_param->panel.ckv0_on_2c      );
	pr_debug("[%s]param->panel.ckv0_off_2c      = 0x%02X\n",
			__func__, clkchg_param->panel.ckv0_off_2c     );
	pr_debug("[%s]param->panel.rgb_chgen_on_2c  = 0x%02X\n",
			__func__, clkchg_param->panel.rgb_chgen_on_2c );
	pr_debug("[%s]param->panel.rgb_chgen_off_2c = 0x%02X\n",
			__func__, clkchg_param->panel.rgb_chgen_off_2c);
	pr_debug("[%s]param->panel.rgb_eq1_2c       = 0x%02X\n",
			__func__, clkchg_param->panel.rgb_eq1_2c      );
	pr_debug("[%s]param->panel.rgb_eq2_2c       = 0x%02X\n",
			__func__, clkchg_param->panel.rgb_eq2_2c      );
	pr_debug("[%s]param->panel.rgb_eq3_2c       = 0x%02X\n",
			__func__, clkchg_param->panel.rgb_eq3_2c      );
	pr_debug("[%s]param->panel.oscd_adj         = 0x%02X\n",
			__func__, clkchg_param->panel.oscd_adj        );
}
		...
		struct drm_file *file)
{
	...
	fpslow_param = &priv->fpslow_param;
	if (!fpslow_param) {
		return -EINVAL;
	}

	memcpy(fpslow_param, data, sizeof(int));
	msm_set_fps_low_base(*fpslow_param);
