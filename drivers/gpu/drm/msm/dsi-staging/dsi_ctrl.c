...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00012 */
#define TX_DUMP_MAXLEN 130

static void dsi_message_tx_dump(const struct mipi_dsi_msg *msg)
{
	char* pos;
	char tmp[3];
	char str[TX_DUMP_MAXLEN*3+1];
	int i;
	...
	if ((pos != NULL) &&
		(msg->tx_len <= TX_DUMP_MAXLEN)) {
		if ((int)msg->tx_len == (int)1) {
			DRM_DEBUG_DSI("%s: type = %02x(%s), addr = %02x", __func__, msg->type, (msg->flags & MIPI_DSI_MSG_USE_LPM) ? "LP" : "HS", *pos);
		} else {
			memset(tmp, 0, sizeof(tmp));
			memset(str, 0, sizeof(str));
			for (i = 1; i < (int)msg->tx_len; i++) {
				if ((pos + i) == NULL)
					return;
				snprintf(tmp, 3, "%02x", *(pos + i));
				strncat(str, " ", 1);
				strncat(str, tmp, 2);
			}
			DRM_DEBUG_DSI("%s: type = %02x(%s), addr = %02x, value =%s", __func__, msg->type, (msg->flags & MIPI_DSI_MSG_USE_LPM) ? "LP" : "HS", *pos, str);
		}
	}
	return;
}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00012 */
	if (drm_debug & DRM_UT_DSI) {
		dsi_message_tx_dump(msg);
	}
#endif /* CONFIG_SHARP_DISPLAY */
