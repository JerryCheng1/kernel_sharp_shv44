...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00065 */
...
#include "sde_connector.h"
#endif /* CONFIG_SHARP_DISPLAY */
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_SHARP_BOOT) /* CUST_ID_00071 */
#include <soc/qcom/sharp/sh_boot_manager.h>
#endif /* CONFIG_SHARP_BOOT *//* CONFIG_SHARP_DISPLAY */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
static int dsi_panel_bl_boost(struct dsi_panel *panel, u32 onoff);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
static int dsi_panel_update_backlight_boost(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;
	...
	mutex_lock(&panel->curr_boost_lock);
	prev_state = bl->last_level > 0 ? true : false;
	bkl_on = bl_lvl > 0 ? true : false;
	bl->last_level = bl_lvl;

	//bkl_level OFF->ON or bkl_level ON->OFF
	if ((bkl_on != prev_state) && bl->curr_boosted) {
		dsi_panel_bl_boost(panel, false);
		bl->curr_boosted   = false;
		bl->curr_boost_req = false;
	}
	...
	mutex_unlock(&panel->curr_boost_lock);
	return rc;
}
#endif /* CONFIG_SHARP_DISPLAY */
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_WLED:
	case DSI_BACKLIGHT_DCS:
		rc = dsi_panel_update_backlight_boost(panel, bl_lvl);
		break;
#else /* CONFIG_SHARP_DISPLAY */
	case DSI_BACKLIGHT_WLED:
		rc = backlight_device_set_brightness(bl->raw_bd, bl_lvl);
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->vcc_gpio = utils->get_named_gpio(utils->data,
				...
				int topology_override)
{
	...
	int rc = 0;
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_SHARP_BOOT) /* CUST_ID_00071 */
	unsigned long boot_mode = 0;
#endif /* CONFIG_SHARP_DISPLAY *//* CONFIG_SHARP_BOOT */
...
#if defined(CONFIG_SHARP_DISPLAY) && defined(CONFIG_SHARP_BOOT) /* CUST_ID_00053 *//* CUST_ID_00071 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 *//* CUST_ID_00010 */
	if (panel->lp11_init) {
...
int dsi_panel_backlight_mfr(struct dsi_panel *panel, int value)
{
	...
	if (!connector) {
		pr_err("[%s]Invalid drm_connector\n", __func__);
		return -EINVAL;
	}
	...
	if (sde_conn->last_panel_power_mode == SDE_MODE_DPMS_LP1) {
		pr_debug("%s:power_mode = SDE_MODE_DPMS_LP1\n", __func__);
		return ret;
	}
...
int dsi_panel_aod_enable(struct dsi_panel *panel, bool enable)
{
	...
	if (!enable) {
		mutex_lock(&display->display_lock);
		if (panel->mfr_idx >= 0 && panel->mfr_idx < panel->num_mfr) {
			mfr_cmds = &panel->mfr_config[panel->mfr_idx].mfr_cmds;

			pr_debug("%s: count=%d\n", __func__, mfr_cmds->count);

			if (mfr_cmds->count) {
				drm_cmn_panel_cmds_transfer(display,
					mfr_cmds->cmds, mfr_cmds->count);
			} else {
				pr_err("%s:mfr_cmds is zero.\n", __func__);
			}
		}
		mutex_unlock(&display->display_lock);
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
static struct dsi_panel *to_dsi_panel(struct backlight_device *bd)
{
	...
	if (!c_conn) {
		pr_err("%s: sde_connector is null\n",__func__);
		return NULL;
	}
					...
					u32 *boost_req, u32 *boosted)
{
	...
	panel = to_dsi_panel(bd);
	if (!bd || !panel) {
		pr_err("%s: Invalid input data\n", __func__);
		return -EINVAL;
	}
	if ((!boost_req) || (!boosted)) {
	...
	*boost_req = bl->curr_boost_req;
	*boosted = bl->curr_boosted;
	return 0;
}
...
int dsi_panel_request_boost(struct backlight_device *bd, u32 onoff)
{
	...
	if ((onoff) && (bl->last_level == 0)) {
		pr_debug("%s: skip curr_boost on\n",__func__);
		return 0;
	}

	cancel_delayed_work_sync(&panel->curr_boost_work);
	bl->curr_boost_req = (bool)onoff;
	queue_delayed_work(panel->ordered_workqueue, &panel->curr_boost_work, 0);
...
static int dsi_panel_bl_boost(struct dsi_panel *panel, u32 onoff)
{
	...
	if (onoff) {
		// curr_boost on
		bl_lvl = BKL_CURR_MAX_BRIGHTNESS;
	} else {
		// curr_boost off
		bl_lvl = bl->last_level;
	}
...
static void dsi_panel_curr_boost_work(struct work_struct *work)
{
	...
	panel = container_of(dw,
		struct dsi_panel, curr_boost_work);
	if (!panel) {
		pr_err("%s: Invalid panel data\n", __func__);
		return;
	}
	...
	mutex_lock(&panel->curr_boost_lock);
	if (bl->curr_boost_req == true) {
		bl->curr_boost_req = false;
		if (bl->last_level) {
			//set curr_boost on
			rc = dsi_panel_bl_boost(panel, true);
			if (!rc) {
				bl->curr_boosted = true;
				queue_delayed_work(panel->ordered_workqueue,
						&panel->curr_boost_work,
						msecs_to_jiffies(cancel_delay_ms));
			} else {
				pr_err("%s: curr_boost failed err=%d.\n",
							__func__, rc);
			}
		}
	}
...
int dsi_panel_boost_work_init(struct backlight_device *bd, bool Initialize)
{
	int rc = 0;
	struct dsi_panel *panel = NULL;
	...
	if (Initialize) {
		mutex_init(&panel->curr_boost_lock);
		panel->ordered_workqueue =
			alloc_ordered_workqueue("curr_boost_workqueue", 0);
		if (!panel->ordered_workqueue) {
			rc = -ENOMEM;
			pr_err("%s: alloc_ordered_workqueue failed\n", __func__);
			return rc;
		}
		INIT_DELAYED_WORK(&panel->curr_boost_work, dsi_panel_curr_boost_work);
	} else {
		if (panel->ordered_workqueue) {
			cancel_delayed_work_sync(&panel->curr_boost_work);
			destroy_workqueue(panel->ordered_workqueue);
		}
		mutex_destroy(&panel->curr_boost_lock);
	}
	return rc;
}
#endif /* CONFIG_SHARP_DISPLAY */
