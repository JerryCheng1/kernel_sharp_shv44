...
static int kgsl_open(struct inode *inodep, struct file *filep)
{
	...
	if(p_smem != 0){
		if(( p_smem->sh_boot_mode == SH_BOOT_MODE_DIAG)||(p_smem->sh_boot_mode == SH_BOOT_MODE_FORCE_FUNC)){

			if(!strncmp(current->comm, "shdiag_", 7)){
			} else if(!strncmp(current->group_leader->comm, "omx@1.0-service", 15)){
			} else{
				pr_err(" err: kgsl_open is called by unknown(%s)\n", current->comm);
				return -1;
			}
		}
	}
