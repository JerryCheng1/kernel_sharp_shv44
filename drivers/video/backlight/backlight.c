...
static ssize_t backlight_curr_boost_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct backlight_device *bd = to_backlight_device(dev);
	int boost_req = 0;
	...
	if ((bd) && (bd->ops->get_curr_boost)) {
		bd->ops->get_curr_boost(bd, &boost_req, &boosted);
	}
...
static ssize_t backlight_curr_boost_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct backlight_device *bd = to_backlight_device(dev);
	int data, rc;
	...
	if ((bd) && (bd->ops->req_curr_boost)) {
		rc = bd->ops->req_curr_boost(bd, data);
	}
	return count;
}
/* sysfs attributes exported by curr_boost */
static DEVICE_ATTR(curr_boost, 0664,
		backlight_curr_boost_show, backlight_curr_boost_store);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
extern int dsi_panel_boost_work_init(struct backlight_device *bd, bool Initialize);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
	dsi_panel_boost_work_init(new_bd, true);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
	dsi_panel_boost_work_init(bd, false);
#endif /* CONFIG_SHARP_DISPLAY */
