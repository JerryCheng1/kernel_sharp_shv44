...
#include "../../leds/leds.h"
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
#include <soc/qcom/sh_smem.h>
#include "../../gpu/drm/msm/sharp/msm_drm_context.h"
...
#define   WLED5_SAFETY_TIMER_DISABLE	0x88
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00028 */
struct wled_linear_param {
	int	x;
	int	y;
};

struct wled_linear_params {
	int	cnt;
	struct wled_linear_param *param;
};
#endif /* CONFIG_LEDS_SHARP */
...
struct wled {
	...
	u32 leds_per_string;
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00028 */
	struct wled_linear_params	linear_params;
#endif /* CONFIG_LEDS_SHARP */
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
	bool is_limit;
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
static int limit_brightness = 0xCCC;
module_param(limit_brightness, int, 0600);
static int flash_nolimit = 0;
module_param(flash_nolimit, int, 0600);
#endif /* CONFIG_LEDS_SHARP */

static int wled_flash_setup(struct wled *wled);

#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00028 */
static u32 wled_brightness_to_bl(int value,
		struct wled_linear_params *linear_params);
static int wled_parse_brightness(struct wled *wled);
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00028 */
	brightness = wled_brightness_to_bl(brightness, &wled->linear_params);
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
	if (wled->is_limit && brightness > limit_brightness)
		brightness = limit_brightness;
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
	if (wled->is_limit && flash_nolimit == 0)
		fs_current_max = 20;
	pr_debug("%s:fs_current_max=%d\n", __func__, fs_current_max);
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
static int wled_switch_brightness_off(struct backlight_device *bl)
{
	struct wled *wled = NULL;
	struct led_classdev *sw_cdev;
	unsigned int r_val;
	...
	wled = bl_get_data(bl);
	if (!wled) {
		pr_err("%s:wled is null\n",__func__);
		return -EINVAL;
	}
	sw_cdev = &wled->switch_cdev;
	if (!sw_cdev->name) {
		pr_err("%s:cdev is null\n",__func__);
		return -EINVAL;
	}
	regmap_read(wled->regmap,
		wled->sink_addr + WLED5_SINK_FLASH_CTL_REG, &r_val);
	if (!r_val) {
		pr_debug("%s:already off\n",__func__);
		return 0;
	}
	wled_switch_brightness_set(sw_cdev, 0);
	return 0;
}
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	/* Disable flash and pre_flash safety timers */
	val = WLED5_SAFETY_TIMER_DISABLE;
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00046 */
	if (wled->is_limit)
		wled->cfg.boost_i_limit = 2;
#endif /* CONFIG_LEDS_SHARP */
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00028 */
	rc = wled_parse_brightness(wled);
#endif /* CONFIG_LEDS_SHARP */
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00037 */
	.flash_off = wled_switch_brightness_off,
#endif /* CONFIG_LEDS_SHARP */
};
...
static int wled_probe(struct platform_device *pdev)
{
	...
	if (sh_smem_addr != NULL) {
		shdisp_boot_ctx = (struct shdisp_boot_context *)(sh_smem_addr->shdisp_data_buf);
		wled->is_limit = shdisp_boot_ctx->backlight_limit;
	}
	pr_info("%s:is_limit=%d\n", __func__, wled->is_limit);
#endif /* CONFIG_LEDS_SHARP */
