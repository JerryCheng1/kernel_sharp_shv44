...
int dwc3_otg_is_usb_host_running(bool isUnLock)
{
	...
	/* COORDINATOR Qualcomm_CS2.14/PreCS2.7 BUILDERR MODIFY start */
//	if (the_dwc3_otg->otg_state >= OTG_STATE_A_IDLE && atomic_read(&dwc3_usb_host_enable) == 1) {
	...
	/* COORDINATOR Qualcomm_CS2.14/PreCS2.7 BUILDERR MODIFY end */
...
void dwc3_otg_limitlock(void)
{
	...
	for_each_possible_cpu(cpu_id) {
		if (cpu_id <= 3)
			per_cpu(limitlock_freq, cpu_id) = 940800;
		if (cpu_id >=  4)
			per_cpu(limitlock_freq, cpu_id) = 940800;
	}
