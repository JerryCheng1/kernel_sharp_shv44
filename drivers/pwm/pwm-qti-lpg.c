...
#define QPNP_SET_PAUSE_CNT(to_pause_cnt, from_pause, ramp_ms) \
	(to_pause_cnt = (from_pause / (ramp_ms ? ramp_ms : 1)))
		...
		unsigned int *pattern, unsigned int length)
{
	...
	for (i = 0; i < length; i++) {
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00036 */
		if (lpg->ramp_config.direct_writing_mode) {
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00036 */
int qpnp_lpg_pwm_set_custom_pattern(struct pwm_chip *pwm_chip,
				struct pwm_device *pwm,
				struct pwm_state *state)
{
	...
	if ((state->pattern_length == 0) ||
	    (state->pattern == NULL) ||
	    (state->ramp_step_ms == 0)) {
		dev_err(pwm_chip->dev, "not set custam pattern\n");
		return -ENODEV;
	}
	...
	//duty_pcts
	ramp->pattern = state->pattern;
	ramp->pattern_length = state->pattern_length;

	//ramp_step_ms
	ramp->step_ms = state->ramp_step_ms;

	//start_idx
	ramp->lo_idx = state->lo_idx;
	ramp->hi_idx = state->lo_idx + state->pattern_length - 1;

	//pause_hi
	QPNP_SET_PAUSE_CNT(ramp->pause_hi_count,
			state->pause_hi, ramp->step_ms);
	if (ramp->pause_hi_count > PM_PWM_MAX_PAUSE_CNT)
		ramp->pause_hi_count = PM_PWM_MAX_PAUSE_CNT;

	//puase_lo
	QPNP_SET_PAUSE_CNT(ramp->pause_lo_count,
			state->pause_lo, ramp->step_ms);
	if (ramp->pause_lo_count > PM_PWM_MAX_PAUSE_CNT)
		ramp->pause_lo_count = PM_PWM_MAX_PAUSE_CNT;

	//make from lut_flags
	ramp->pattern_repeat = (state->lut_flags & 0x01) ? true : false;	//LOOP
	ramp->ramp_dir_low_to_hi = (state->lut_flags & 0x02) ? true : false;	//RAMP_UP
	ramp->toggle = (state->lut_flags & 0x04) ? true : false;		//REVERSE
	ramp->direct_writing_mode = (state->lut_flags & 0x40) ? true : false;

	return 0;
}

int qpnp_lpg_pwm_get_state_to_ramp(struct pwm_device *pwm, struct pwm_state *state)
{
	struct pwm_chip *pwm_chip;
	struct qpnp_lpg_channel *lpg;
	struct lpg_ramp_config *ramp;
	...
	//duty_pcts
	state->pattern = ramp->pattern;
	state->pattern_length = ramp->pattern_length;
	//ramp_step_ms
	state->ramp_step_ms = ramp->step_ms;
	//start_idx
	state->lo_idx = ramp->lo_idx;
	//pause_hi
	state->pause_hi = ramp->pause_hi_count * ramp->step_ms;
	//pause_lo
	state->pause_lo = ramp->pause_lo_count * ramp->step_ms;
	//make lut_flags
	state->lut_flags  = (ramp->pattern_repeat) ? 0x01 : 0x00;	//LOOP
	state->lut_flags |= (ramp->ramp_dir_low_to_hi) ? 0x02 : 0x00;	//RAMP_UP
	state->lut_flags |= (ramp->toggle) ? 0x04 : 0x00;		//REVERSE
	state->lut_flags |= (ramp->pause_hi_count) ? 0x08 : 0x00;	//HI_EN
	state->lut_flags |= (ramp->pause_lo_count) ? 0x10 : 0x00;	//LO_EN
	return 0;
}
#endif /* CONFIG_LEDS_SHARP */
...
#endif
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00036 */
	.set_cust_pattern = qpnp_lpg_pwm_set_custom_pattern,
	.get_ramp_config = qpnp_lpg_pwm_get_state_to_ramp,
#endif /* CONFIG_LEDS_SHARP */
	.owner = THIS_MODULE,
};
...
#ifdef CONFIG_LEDS_SHARP /* CUST_ID_00036 */
		lpg->max_pattern_length = PWM_STATE_PATTERN_LENGTH_MAX;
#else /* CONFIG_LEDS_SHARP */
