...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
...
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
...
#define HAP_VMAX_MIN_REG        ((HAP_VMAX_MV_LSB / HAP_VMAX_MV_LSB) << HAP_VMAX_MV_SHIFT)
#define HAP_VMAX_MAX_REG        ((HAP_VMAX_MV_MAX / HAP_VMAX_MV_LSB) << HAP_VMAX_MV_SHIFT)
#ifdef CONFIG_SHARP_BOOT
static unsigned long bootmode = SH_BOOT_NORMAL;
static bool qti_hap_set_smem_param = false;
#endif /* CONFIG_SHARP_BOOT */
static bool qti_hap_module_en_enabled = false;
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
...
static void qti_haptics_module_en_custom(bool en)
{
	if (en) {
		if(qti_hap_module_en_enabled == false) {
		...
		if(qti_hap_module_en_enabled == true) {
			qpnp_hap_pm_qos_end();
			#ifdef CONFIG_SHARP_SHTERM
...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
#ifdef CONFIG_SHARP_BOOT
static int qti_haptics_smem_config(struct qti_hap_chip *chip)
{
	int rc = 0;
	int i;
	...
	if (!p_sh_smem_common_type) {
		dev_dbg(chip->dev, "[qpnp_hap]p_sh_smem_common_type is null\n");
		return 1;
	}
	for (i=0; i<4; i++) {
		param[i] = (u8)p_sh_smem_common_type->shdiag_vib_param[i];
		dev_dbg(chip->dev, "[qpnp_hap]smem param[%d]:0x%02X, ", i, param[i]);
	}
	dev_dbg(chip->dev, "\n");
	
	if (!param[0] && !param[1] && !param[2] && !param[3]) {
		dev_dbg(chip->dev, "[qpnp_hap]org_param write, qti_hap_set_smem_param:%d\n", qti_hap_set_smem_param);
		if (qti_hap_set_smem_param) {
			rc = qti_haptics_write(chip, REG_HAP_AUTO_RES_CFG, &chip->auto_res_reg, 1);
			if (rc) {
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_LRA_AUTO_RES_REG org_param write err rc:%d\n", rc);
				return rc;
			}
			rc = qti_haptics_write(chip, REG_HAP_VMAX_CFG, &chip->vmax_reg, 1);
			if (rc){
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_VMAX_REG org_param write err rc:%d\n", rc);
				return rc;
			}
			rc = qti_haptics_write(chip, REG_HAP_RATE_CFG1, &chip->rate_cfg1_reg, 1);
			if (rc){
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_RATE_CFG1_REG org_param write err rc:%d\n", rc);
				return rc;
			}
			rc = qti_haptics_write(chip, REG_HAP_RATE_CFG2, &chip->rate_cfg2_reg, 1);
			if (rc){
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_RATE_CFG2_REG org_param write err rc:%d\n", rc);
				return rc;
			}
			qti_hap_set_smem_param = false;
		} 
	} else {
		dev_dbg(chip->dev, "[qpnp_hap]smem_param write\n");
		qti_hap_set_smem_param = true;

		rc = qti_haptics_write(chip, REG_HAP_AUTO_RES_CFG, &param[0], 1);
		if (rc) {
			dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_LRA_AUTO_RES_REG smem_param write err rc:%d\n", rc);
			return rc;
		}
		if ((param[1] >= HAP_VMAX_MIN_REG) && (param[1] <= HAP_VMAX_MAX_REG)) {
			rc = qti_haptics_write(chip, REG_HAP_VMAX_CFG, &param[1], 1);
			if (rc) {
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_VMAX_REG smem_param write err rc:%d\n", rc);
				return rc;
			}
		} else {
			dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_VMAX_REG smem_param write param err param:0x%02x\n", rc);
		}
		rc = qti_haptics_write(chip, REG_HAP_RATE_CFG1, &param[2], 1);
		if (rc) {
			dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_RATE_CFG1_REG smem_param write err rc:%d\n", rc);
			return rc;
		}
		rc = qti_haptics_write(chip, REG_HAP_RATE_CFG2, &param[3], 1);
		if (rc) {
			dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_RATE_CFG2_REG smem_param write err rc:%d\n", rc);
			return rc;
		}
	}
	return rc;
...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
	qti_haptics_module_en_custom(en);
	if (en) {
		qti_hap_module_en_enabled = true;
	} else {
		qti_hap_module_en_enabled = false;
	}
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
...
#ifndef CONFIG_INPUT_QTI_SHARP_HAPTICS
		rc = qti_haptics_config_vmax(chip, play->vmax_mv);
#else /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
		...
		struct ff_effect *effect, struct ff_effect *old)
{
	...
	if ((bootmode == SH_BOOT_D) || (bootmode == SH_BOOT_F_F)) {
		rc = qti_haptics_smem_config(chip);
		if (rc < 0) {
			dev_err(chip->dev, "smem config failed, rc=%d\n", rc);
			return rc;
		}
	}
#endif /* CONFIG_SHARP_BOOT */
...
static int qti_haptics_hw_init(struct qti_hap_chip *chip)
{
	...
	chip->auto_res_reg = ((config->lra_auto_res_mode << HAP_AUTO_RES_MODE_SHIFT)
						 | HAP_CAL_EOP_EN_BIT | HAP_CAL_OPT3_EVERY_8_PERIOD);
	chip->vmax_reg     = ((config->vmax_mv / HAP_VMAX_MV_LSB) << HAP_VMAX_MV_SHIFT);

	tmp = (config->play_rate_us / HAP_PLAY_RATE_US_LSB);
	chip->rate_cfg1_reg = tmp & 0xff;
	chip->rate_cfg2_reg = (tmp >> 8) & 0xf;
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
...
static enum hrtimer_restart qti_hap_stop_timer(struct hrtimer *timer)
{
	...
	rc = qti_haptics_module_en(chip, false);
	if (rc < 0)
		dev_err(chip->dev, "Stop module_en failed, rc=%d\n", rc);
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
