...
void shtps_sleep(struct shtps_rmi_spi *ts, int on)
{
	...
	if(	(ts->state_mgr.state == SHTPS_STATE_IDLE) ||
		(ts->state_mgr.state == SHTPS_STATE_WAIT_WAKEUP) ||
		(ts->state_mgr.state == SHTPS_STATE_WAIT_READY) ){
		return;
	}
...
static void shtps_rmi_close(struct input_dev *dev)
{
	...
	shtps_func_request_sync(ts, SHTPS_FUNC_REQ_EVENT_CLOSE);
}
...
fail_init:
	#if defined(SHTPS_GUARANTEE_SPI_ACCESS_IN_WAKE_ENABLE)
		pm_qos_remove_request(&ts->deter_suspend_spi.pm_qos_lock_idle);
		wake_lock_destroy(&ts->deter_suspend_spi.wake_lock);
	#endif /* SHTPS_GUARANTEE_SPI_ACCESS_IN_WAKE_ENABLE */
...
static void shtps_deinit_internal_variables(struct shtps_rmi_spi *ts)
{
	...
	if(ts){
		...
		#if defined(SHTPS_PROXIMITY_SUPPORT_ENABLE)
			pm_qos_remove_request(&ts->pm_qos_lock_idle_proximity);
...
static void shtps_deinit_inputdev_key(struct shtps_rmi_spi *ts)
{
	SHTPS_LOG_FUNC_CALL();
	if(ts && ts->input_key){
		input_unregister_device(ts->input_key);
		ts->input_key = NULL;
	}
}
#endif /* SHTPS_SUPPORT_GESTURE_NOTIFY */
