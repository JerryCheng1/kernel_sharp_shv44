...
#define SHTPS_SERIAL_NUMBER_ALL_SIZE	0x70
...
static int shtps_fwctl_s3700_map_construct_relating_f12_baseaddress(struct shtps_fwctl_info *fc_p);
...
static int shtps_fwctl_s3700_map_construct(struct shtps_fwctl_info *fc_p, int func_check)
{
	...
	for(page = 0; page <= page_max; page++){
		...
		for(i = 0xE9;i > 0x0a;i-=sizeof(pdt)){
			...
			case 0x12:
				...
				rc = shtps_fwctl_s3700_map_construct_relating_f12_baseaddress(fc_p);
				if(rc)	goto err_exit;
			...
			case 0x54:
				...
				SHTPS_LOG_DBG_PRINT("-------------------------------------------------\n");
				SHTPS_LOG_DBG_PRINT("ReceiverElectrodeCount        : 0x%02x\n", F54_QUERY_RECEIVERELECTRODECOUNT(fc_p->map_p->fn54.query.data));
				SHTPS_LOG_DBG_PRINT("TransmitterElectrodeCount     : 0x%02x\n", F54_QUERY_TRANSMITTERELECTRODECOUNT(fc_p->map_p->fn54.query.data));
				...
				SHTPS_LOG_DBG_PRINT("TouchControllerFamily         : 0x%02x\n", F54_QUERY_TOUCHCONTROLLERFAMILY(fc_p->map_p->fn54.query.data));
				SHTPS_LOG_DBG_PRINT("HasPixelTouchThresholdTuning  : 0x%02x\n", F54_QUERY_HASPIXELTOUCHTHRESHOLDTUNING(fc_p->map_p->fn54.query.data));
				SHTPS_LOG_DBG_PRINT("HasSensorAssignment           : 0x%02x\n", F54_QUERY_HASSENSORASSIGNMENT(fc_p->map_p->fn54.query.data));
				...
				SHTPS_LOG_DBG_PRINT("RSVD                          : 0x%02x\n", F54_QUERY_RSVD(fc_p->map_p->fn54.query.data));
				...
				SHTPS_LOG_DBG_PRINT("CurveCompensationMode         : 0x%02x\n", F54_QUERY_CURVECOMPENSATIONMODE(fc_p->map_p->fn54.query.data));
				...
				SHTPS_LOG_DBG_PRINT("HasCMNMaximum                 : 0x%02x\n", F54_QUERY_HASCMNMAXIMUM(fc_p->map_p->fn54.query.data));
				...
				SHTPS_LOG_DBG_PRINT("HasMMStateMachine             : 0x%02x\n", F54_QUERY_HASMMSTATEMACHINE(fc_p->map_p->fn54.query.data));
				SHTPS_LOG_DBG_PRINT("HasSignalClarity              : 0x%02x\n", F54_QUERY_HASSIGNALCLARITY(fc_p->map_p->fn54.query.data));
				SHTPS_LOG_DBG_PRINT("HasBurstSpanmetric            : 0x%02x\n", F54_QUERY_HASBURSTSPANMETRIC(fc_p->map_p->fn54.query.data));
	...
	if(func_check){
		...
		if(rc == 0){
			u8 buf[8];
			rc = M_READ_PACKET_FUNC(fc_p, fc_p->map_p->fn54.ctrlBase + SHTPS_REG_F54_ANALOG_CTRL113_OFFSET, buf, sizeof(buf));
			if(rc == 0){
				fc_p->map_p->reg_fn54_ctrl113_TX_thermal_update_interval_def[0] = buf[4];
				fc_p->map_p->reg_fn54_ctrl113_TX_thermal_update_interval_def[1] = buf[5];
				fc_p->map_p->reg_fn54_ctrl113_RX_thermal_update_interval_def[0] = buf[6];
				fc_p->map_p->reg_fn54_ctrl113_RX_thermal_update_interval_def[1] = buf[7];
			}
		}
...
static int shtps_fwctl_s3700_make_serial_number_data(u8 *data_p, int data_length, u8 *buf_p)
{
	...
	for(i = 0; i < (sizeof(read_info) / sizeof(struct read_info_s)); i++){
		...
		if(read_info[i].search_str != NULL){
			...
			if(str_p == NULL){
				SHTPS_LOG_ERR_PRINT("%s(): serial number tag \"%s\" not found. fill with 0.\n", __func__, read_info[i].search_str);
				// fill with '0'
...
static int shtps_fwctl_s3700_glove_enable(struct shtps_fwctl_info *fc_p)
{
	...
	if(fc_p->map_p->fn12.ctrl.support_object_glove != 0){
			...
			if(SHTPS_PRM_GLOVE_CHANGE_TUI_ENABLE == 1){
				u8 buf[8];
				ret = M_READ_PACKET_FUNC(fc_p, fc_p->map_p->fn54.ctrlBase + SHTPS_REG_F54_ANALOG_CTRL113_OFFSET, buf, sizeof(buf));
				if(ret == 0){
					...
					ret = M_WRITE_PACKET_FUNC(fc_p, fc_p->map_p->fn54.ctrlBase + SHTPS_REG_F54_ANALOG_CTRL113_OFFSET, buf, sizeof(buf));
...
static int shtps_fwctl_s3700_glove_disable(struct shtps_fwctl_info *fc_p)
{
	...
	if(fc_p->map_p->fn12.ctrl.support_object_glove != 0){
			...
			if(SHTPS_PRM_GLOVE_CHANGE_TUI_ENABLE == 1){
				...
				ret = M_READ_PACKET_FUNC(fc_p, fc_p->map_p->fn54.ctrlBase + SHTPS_REG_F54_ANALOG_CTRL113_OFFSET, buf, sizeof(buf));
				if(ret == 0){
					buf[4] = fc_p->map_p->reg_fn54_ctrl113_TX_thermal_update_interval_def[0];
					buf[5] = fc_p->map_p->reg_fn54_ctrl113_TX_thermal_update_interval_def[1];
					buf[6] = fc_p->map_p->reg_fn54_ctrl113_RX_thermal_update_interval_def[0];
					buf[7] = fc_p->map_p->reg_fn54_ctrl113_RX_thermal_update_interval_def[1];
					ret = M_WRITE_PACKET_FUNC(fc_p, fc_p->map_p->fn54.ctrlBase + SHTPS_REG_F54_ANALOG_CTRL113_OFFSET, buf, sizeof(buf));
					if(ret == 0)	ret = shtps_fwctl_s3700_command_force_update(fc_p);
					if(ret == 0)	ret = shtps_fwctl_s3700_f54_command_completion_wait(fc_p, SHTPS_F54_COMMAND_WAIT_POLL_COUNT, SHTPS_F54_COMMAND_WAIT_POLL_INTERVAL);
				}
