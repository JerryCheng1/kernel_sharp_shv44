...
static void fc8350_set_filter(HANDLE handle, DEVICEID devid, u16 filter_bw)
{
	...
	fc8350_write(handle, devid, 0xCA, 0x80);/*agc manual hold 20181030*/
	fc8350_write(handle, devid, 0xC3, 0x16);/*lna max hold 20181030*/
	fc8350_write(handle, devid, 0xC8, 0x19);/*dm max hold 20181030*/
		...
		enum BROADCAST_TYPE broadcast)
{
	...
	if (hOpen->driver_config.b_ext_lna)
		fc8350_write(handle, devid, 0xbd, 0x83);
	else
	...
	if (hOpen->driver_config.b_ext_lna_pol_high)
		fc8350_write(handle, devid, 0xf2, 0x10);
	else
	...
	fc8350_bb_write(handle, devid, 0x00cd, 0x0f);

	fc8350_bb_write(handle, devid, 0x00cb, 0xcf);

	fc8350_bb_write(handle, devid, 0xc0bd, 0x83);
	fc8350_bb_write(handle, devid, 0xc0be, 0x07);
	fc8350_bb_write(handle, devid, 0xc0c1, 0xce);
	fc8350_bb_write(handle, devid, 0xc0c2, 0xe2);
	fc8350_bb_write(handle, devid, 0xc0f2, 0x00);
	fc8350_bb_write(handle, devid, 0xc0f3, 0x0f);
	fc8350_bb_write(handle, devid, 0xc0f5, 0x84);
