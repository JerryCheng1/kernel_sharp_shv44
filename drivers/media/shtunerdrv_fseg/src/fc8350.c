...
static const char *reg_id_l13 = "pm8150_l13";
...
static int tuner_vreg_enable(void)
{
	...
	reg_l13 = regulator_get(fc8350_misc_device.this_device, reg_id_l13);
	if (IS_ERR(reg_l13)) {
	...
	ret = regulator_set_voltage(reg_l13, min_uV_18v, max_uV_18v);
	if (ret != 0) {
		printk("Unable to set %s regulator voltage[ret:%d]\n", reg_id_l13, ret);
		regulator_put(reg_l13);
		misc_deregister(&fc8350_misc_device);
		return ret;
	}
