...
/* SHLOCAL_CAMERA_DRIVERS-> */
			{
				...
				CAM_ERR(CAM_OIS, "header.count=%d", cam_cmd_i2c_random_wr->header.count);
...
/* SHLOCAL_CAMERA_DRIVERS-> */
		CAM_ERR(CAM_OIS, "o_ctrl->i2c_init_data.is_settings_valid %d",o_ctrl->i2c_init_data.is_settings_valid);
		if(o_ctrl->i2c_init_data.is_settings_valid == 0){
			break;
		}
/* SHLOCAL_CAMERA_DRIVERS<- */
...
/* SHLOCAL_CAMERA_DRIVERS-> */
		...
		
		{
			...
			for (cnt = 0; cnt < (cam_cmd_i2c_random_wr->header.count); cnt++) {
				CAM_ERR(CAM_OIS, "reg_addr=0x%0x reg_data=0x%0x", cam_cmd_i2c_random_wr->random_wr_payload[cnt].reg_addr, cam_cmd_i2c_random_wr->random_wr_payload[cnt].reg_data);
			}
		}
