...
static unsigned int sh_max_freq_gold_plus = UINT_MAX;
unsigned int sh_get_max_freq(struct cpufreq_policy *policy)
{
	unsigned int ret = UINT_MAX;

	if (policy->cpu <= 3)
		ret = sh_max_freq_silver;
	if (policy->cpu >= 4 && policy->cpu <= 6)
		ret = sh_max_freq_gold;
	if (policy->cpu == 7)
		ret = sh_max_freq_gold_plus;
...
void sh_set_max_freq(struct cpufreq_policy *policy, unsigned int max_freq)
{
	...
	if (policy->cpu == 7)
		sh_max_freq_gold_plus = max_freq;
}
